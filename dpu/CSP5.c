...
static ans_t __imp_csp5_2(sysname_t tasklet_id, node_t root, node_t second_root) {
    ...
    for (node_t j = 0; j < common_size; j++) {
        ...
        node_t common_size2 = intersect_seq_buf_thresh_withcbuf(tasklet_buf, mram_buf[tasklet_id], common_size, &col_idx[third_root_begin], third_root_end - third_root_begin, mram_buf[tasklet_id + NR_TASKLETS], INVALID_NODE);
        ans += ((common_size - 1) *(third_root_end - third_root_begin - 2) - common_size2 );
    }
    return ans;
}

static ans_t __imp_csp5(sysname_t tasklet_id, node_t root) { 
    ...
    for (edge_ptr i = root_begin; i < root_end; i++) {
        ...
        ans += __imp_csp5_2(tasklet_id, root, second_root);
    }
    return ans;
}

extern void csp5(sysname_t tasklet_id) {
    ...
    while (i < root_num) {
        ...
        for (edge_ptr j = root_begin + tasklet_id; j < root_end; j += NR_TASKLETS) {
            ...
            if (second_root >= root) break;
            partial_ans[tasklet_id] += __imp_csp5_2(tasklet_id, root, second_root);
    ...
    for (i += tasklet_id; i < root_num; i += NR_TASKLETS) {
        ...
        timer_start(&cycles[tasklet_id]);
#endif
        ans[i] = __imp_csp5(tasklet_id, root);  // intended DMA
#ifdef PERF
